...
exports.get_page_dates = function(year,month){
    ...
    // 年月最后一天
    var prev = get_month_last_day( pre_date.year , pre_date.month )
    var cur = get_month_last_day( date.year,date.month )
    var next = get_month_last_day( next_date.year , next_date.month )
    
    // 当页面数据
    var prev_month_dates = get_prev_month_dates( prev.day , prev.dater )
    var full_month_dates = get_full_month_dates( cur.dater )
...
// 当前月(满月) YY-MM-DD
function get_full_month_dates (dater){
    var ymdr = ymd(dater),
        counts = ymdr.d                    

    return  get_month_dates( counts , ymdr.d , ymdr.ym , 'active')
